...
if st.session_state["current_page"] == "Overview":
    ...
    with k4:
        st.markdown(
            ...
            f'<div class="kpi-top-row"><span class="kpi-label">Terminations</span>'
            f'<div class="kpi-icon-badge" style="background: rgba(244, 63, 94, 0.15); color: #fb7185;">⚠️</div></div>'
            f'<div class="kpi-value">{terminated_count:,}</div>'
            f'<div class="kpi-pill pill-red">↓ {filtered_term["headcount"].sum():,} Headcount</div></div>',
            unsafe_allow_html=True,
        )
    ...
    with k5:
        st.markdown(
            f'<div class="kpi-card" style="border-color: rgba(168, 85, 247, 0.4);">'
            f'<div class="kpi-top-row"><span class="kpi-label">Lumber Benefits</span>'
            f'<div class="kpi-icon-badge" style="background: rgba(168, 85, 247, 0.15); color: #c084fc;">🌲</div></div>'
            f'<div class="kpi-value">{total_lumber:,}</div>'
            f'<div class="kpi-pill pill-purple">Flores Services</div></div>',
            unsafe_allow_html=True,
        )
    ...
    with c1:
        # UPDATED HEADER HERE
        st.markdown('<div class="chart-container"><div class="chart-header">📈 Monthly Change Request Form</div>', unsafe_allow_html=True)
        selected_overview_month = None
        ...
        if not filtered_crf.empty:
            ...
            fig_trend = px.bar(
                ...
                labels={"month": "", "ticket_count": "Tickets"}
            )
            ...
            trend_selection = st.plotly_chart(
                fig_trend, 
                use_container_width=True,
                on_select="rerun",
                selection_mode="points",
                key="overview_monthly_velocity_chart"
            )

            if trend_selection and "selection" in trend_selection and trend_selection["selection"]["points"]:
                selected_overview_month = trend_selection["selection"]["points"][0].get("x")
        else:
            st.info("No Change Request data found for the active filter selection.")
        st.markdown('</div>', unsafe_allow_html=True)

        if selected_overview_month:
            month_clients_df = filtered_crf[filtered_crf["month"].astype(str).str.lower() == str(selected_overview_month).lower()].copy()
            
            st.markdown(
                f'<div class="kpi-card" style="margin-top: 10px; margin-bottom: 15px; border-color: rgba(168, 85, 247, 0.6); '
                f'display: flex; justify-content: space-between; align-items: center; padding: 16px 24px;">'
                f'<div><span class="kpi-label">Selected Month</span>'
                f'<div style="font-size: 22px; font-weight: 800; color: #c084fc;">{selected_overview_month} {selected_year}</div></div>'
                f'<div style="text-align: right;"><span class="kpi-label">Tickets Recorded</span>'
                f'<div style="font-size: 26px; font-weight: 800; color: #ffffff;">{len(month_clients_df):,} <span style="font-size: 14px; font-weight: 500; color: #94a3b8;">entries</span></div></div></div>',
                unsafe_allow_html=True
            )

            st.markdown(f'<div class="chart-container"><div class="chart-header">📋 Client List for {selected_overview_month} {selected_year}</div>', unsafe_allow_html=True)
            
            display_cols = ["client_name", "partner_name", "category", "configuration_analyst"]
            available_cols = [c for c in display_cols if c in month_clients_df.columns]
            
            st.dataframe(
                month_clients_df[available_cols].rename(columns={
                    "client_name": "Client Name",
                    "partner_name": "Partner Name",
                    "category": "Category",
                    "configuration_analyst": "Config Analyst"
                }),
                use_container_width=True,
                hide_index=True,
                height=300
...
# =========================================================
    ...
    if not filtered_oe.empty and "oe_effective" in filtered_oe.columns:
        ...
        parsed_dates = pd.to_datetime(oe_copy["oe_effective"], errors="coerce")
        oe_copy["oe_effective_clean"] = parsed_dates.dt.month_name().fillna("Unknown")
        
        oe_copy["_month_num"] = parsed_dates.dt.month.fillna(99)
        ...
        eff_df = (
            oe_copy.groupby(["oe_effective_clean", "_month_num", "status_category"], as_index=False)
            .agg(client_count=("client", "count"))
            .sort_values(by=["_month_num", "status_category"])
        )
        ...
        fig_eff = px.bar(
            ...
            labels={
                "oe_effective_clean": "OE Effective Month",
                "client_count": "Client Count",
                "status_category": "Setup Status"
            }
        )
        
        fig_eff.update_xaxes(type="category", categoryorder="array", categoryarray=month_order)
        
        fig_eff.update_traces(
            textposition="inside",
            insidetextanchor="middle",
            textfont=dict(size=12, color="#ffffff", family="Plus Jakarta Sans"),
            hovertemplate="<b>OE Month: %{x}</b><br>Status: %{fullData.name}<br>Clients: <b>%{y}</b><extra></extra>"
        )
        ...
        if selected_effective_date:
            ...
            st.markdown(
                ...
                f'display: flex; justify-content: space-between; align-items: center; padding: 16px 24px;">'
                f'<div><span class="kpi-label">Selected OE Effective Month</span>'
